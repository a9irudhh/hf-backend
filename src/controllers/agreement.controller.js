...
const payEmi = asyncHandler(async (req, res) => {
  try {
      ...
      const [nextDueDate, nextEMIAmount, agreement, remainingMonths] = await Promise.all([
      ...
      if (currentTime <= Number(nextDueDate)) {
          throw new ApiError(400, 
              `Payment not due yet. Next payment due: ${
      ...
      const newRemainingEMIs = await emiManagerContract.getRemainingEMIs(agreementId);
      
      borrowerPayment.transactionHashes.push(
          transferTx.hash, 
          updateTx.hash
      );
      ...
      const responseData = {
          ...
          nextDueDate: new Date(Number(await emiManagerContract.getNextDueDate(agreementId)) * 1000).toISOString(),
